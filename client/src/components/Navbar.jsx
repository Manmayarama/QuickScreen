import React, { useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'
import { assets } from '../assets/assets'
import { MenuIcon, SearchIcon, TicketPlus, XIcon } from 'lucide-react'
import { useClerk, UserButton, useUser } from '@clerk/clerk-react'
import { useAppContext } from '../context/AppContext'

const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false)
  const { user } = useUser()
  const { openSignIn } = useClerk()
  const navigate = useNavigate()
  const {favoriteMovies}=useAppContext()

  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-2  md:rounded-full backdrop-blur bg-black/70'>
      {/*Link tag for logo */}
      <Link className='max-md:flex-1' to='/'>
        <img src={assets.logo} className='w-36 h-auto' />
      </Link>

      {/* Menu Items */}
      <div className={`max-md:fixed max-md:inset-0 max-md:h-screen max-md:w-full
        max-md:font-medium max-md:text-lg z-50 flex flex-col md:flex-row items-center
        max-md:justify-center gap-8 md:px-8 py-3 md:rounded-full backdrop-blur bg-black/70 md:bg-white/10 md:border
        border-gray-300/20 overflow-hidden transition-all duration-300 
        ${isOpen ? 'max-md:opacity-100' : 'max-md:opacity-0 pointer-events-none'}
        md:opacity-100 md:pointer-events-auto`}>

        <XIcon className='md:hidden absolute top-6 right-6 w-6 h-6 cursor-pointer' onClick={() => setIsOpen(!isOpen)} />
        <Link to='/' onClick={() => { scrollTo(0, 0); setIsOpen(false) }}>Home</Link>
        <Link to='/movies' onClick={() => { scrollTo(0, 0); setIsOpen(false) }}>Movies</Link>
        <Link to='' onClick={() => { scrollTo(0, 0); setIsOpen(false) }}>Releases</Link>
        {favoriteMovies.length > 0 && (
          <Link to='/favorite' onClick={() => { scrollTo(0, 0); setIsOpen(false) }}>Favorites</Link>
        )}
        <Link to='/about-us' onClick={() => { scrollTo(0, 0); setIsOpen(false) }}>About Us</Link>
      </div>

      {/* Login/Logout Box */}
      <div className='flex items-center gap-8'>
        <SearchIcon className='max-md:hidden w-6 h-6 cursor-pointer' />
        {
          !user ? (
            <button onClick={openSignIn} className='px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer'>
              Login
            </button>
          ) : (
            <UserButton>
              <UserButton.MenuItems>
                <UserButton.Action label='My Bookings' labelIcon={<TicketPlus width={15} />} onClick={() => navigate('/my-bookings')} />
              </UserButton.MenuItems>
            </UserButton>
          )
        }
      </div>

      {/* Menu Icon for mobile */}
      <MenuIcon className='max-md:ml-4 md:hidden w-8 h-8 cursor-pointer' onClick={() => setIsOpen(!isOpen)} />
    </div>
  )
}

export default Navbar
